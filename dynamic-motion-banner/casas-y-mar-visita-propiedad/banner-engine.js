/**
 * Banner Engine — Casas y Mar · Visita Propiedad
 * Renderiza un canvas 2D con tipografía, imagen, CTA, QR y animaciones.
 */

import casasYMarMotionBanner from "./config.js";

const { client, campaign, destination, theme } = casasYMarMotionBanner;
...
class BannerEngine {
  ...
  async load() {
    const imageUrls = casasYMarMotionBanner.assets.images.map((i) => i.url);
    const fallback = casasYMarMotionBanner.assets.fallbackAssets[0]?.url;
    try {
    ...
    try {
      this.logo = await loadImage(casasYMarMotionBanner.assets.logo.smartUrl || casasYMarMotionBanner.assets.logo.url);
    } catch (e) {
  ...
  draw(alpha = 1) {
    ...
    ctx.fillText(casasYMarMotionBanner.disclaimers.join(" · "), textX, supportY + (isVertical ? W * 0.038 : W * 0.024));
    ctx.restore();
  }
