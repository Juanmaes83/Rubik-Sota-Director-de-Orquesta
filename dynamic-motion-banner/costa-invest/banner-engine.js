/**
 * Banner Engine — Costa Invest · Experiencia Visual de Propiedad
 * Renderiza un canvas 2D con tipografía, imagen, CTA, QR y animaciones.
 */

import costaInvestMotionBanner from "./config.js";

const { client, campaign, destination, theme } = costaInvestMotionBanner;
...
class BannerEngine {
  ...
  async load() {
    const imageUrls = costaInvestMotionBanner.assets.images.map((i) => i.url);
    const fallback = costaInvestMotionBanner.assets.fallbackAssets[0]?.url;
    try {
    ...
    try {
      this.logo = await loadImage(
        costaInvestMotionBanner.assets.logo.smartUrl ||
        costaInvestMotionBanner.assets.logo.url
      );
    } catch (e) {
  ...
  draw(alpha = 1) {
    ...
    ctx.fillText(
      costaInvestMotionBanner.disclaimers.join(" · "),
      textX,
