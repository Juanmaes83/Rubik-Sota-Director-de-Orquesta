/**
 * Banner Engine — Torrevieja Sur · Visita Propiedad
 * Renderiza un canvas 2D con tipografía, imagen, CTA, QR y animaciones.
 */

import torreviejaSurMotionBanner from "./config.js";

const { client, campaign, destination, theme } = torreviejaSurMotionBanner;
...
class BannerEngine {
  ...
  async load() {
    const imageUrls = torreviejaSurMotionBanner.assets.images.map((i) => i.url);
    const fallback = torreviejaSurMotionBanner.assets.fallbackAssets[0]?.url;
    try {
      this.images = await Promise.all(imageUrls.map(loadImage));
    } catch (e) {
      console.warn("Algunas imágenes no cargaron; usando fallback", e);
      try {
    ...
    try {
      this.logo = await loadImage(torreviejaSurMotionBanner.assets.logo.smartUrl || torreviejaSurMotionBanner.assets.logo.url);
    } catch (e) {
  ...
  draw(alpha = 1) {
    ...
    gradTop.addColorStop(0, hexToRgba(theme.background, 0.82));
    ...
    gradBottom.addColorStop(0.55, hexToRgba(theme.background, 0.55));
    gradBottom.addColorStop(1, hexToRgba(theme.background, 0.88));
    ...
    ctx.textAlign = isVertical ? "left" : "left";
    ...
    ctx.fillText(torreviejaSurMotionBanner.disclaimers.join(" · "), textX, supportY + (isVertical ? W * 0.038 : W * 0.024));
    ctx.restore();
  }
