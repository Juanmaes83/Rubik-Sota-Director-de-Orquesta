/**
 * Banner Engine — Casas Espania · Captación de propietarios · Torrevieja
 * Renderiza un canvas 2D con tipografía, imagen, CTA, QR y animaciones.
 */

import casasEspaniaMotionBanner from "./config.js";

const { client, campaign, destination, theme } = casasEspaniaMotionBanner;
...
class BannerEngine {
  ...
  async load() {
    const imageUrls = casasEspaniaMotionBanner.assets.images.map((i) => i.url);
    const fallback = casasEspaniaMotionBanner.assets.fallbackAssets[0]?.url;
    try {
    ...
    try {
      this.logo = await loadImage(
        casasEspaniaMotionBanner.assets.logo.smartUrl ||
        casasEspaniaMotionBanner.assets.logo.url
      );
    } catch (e) {
  ...
  draw(alpha = 1) {
    ...
    ctx.fillText(
      casasEspaniaMotionBanner.disclaimers.join(" Â· "),
      textX,
